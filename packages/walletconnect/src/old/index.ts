...
export interface SolanaWalletConnectSignerInterface extends SolanaWalletConnectInterface {
    ...
    init(client: WalletConnectClient, secretKey?: Buffer | Uint8Array | number[], connection?: Connection): void;
}

export interface SolanaWalletConnectRequesterInterface extends SolanaWalletConnectInterface {
    requestSignTransaction(topic: string, transaction: TransactionRequest, chainId: ChainIdType): Promise<Buffer>;
}

/**
 * Base class for requester and signer implementations
 */
export class SolanaWalletConnect extends EventEmitter implements SolanaWalletConnectInterface {
    _client: WalletConnectClient | null;
    ...
    get client(): WalletConnectClient {
        if (!this._client) throw new Error('SolanaWalletConnect not initialised correctly');
        return this._client;
    }
 ...
 */
export class SolanaWalletConnectRequester extends SolanaWalletConnect implements SolanaWalletConnectRequesterInterface {
    /**
     ...
     */
    public async proposePairing(metadata: AppMetadata, chains?: string[], methods?: string[]): Promise<PairResult> {
 ...
 */
export class SolanaWalletConnectSigner extends SolanaWalletConnect implements SolanaWalletConnectSignerInterface {
    _account: Account | null | undefined;
    _connection: Connection | null | undefined;
     ...
     */
    public async approvePairing(proposal: SessionTypes.Proposal, metadata: AppMetadata): Promise<SessionTypes.Settled> {
        return approvePairing(this.client, proposal, metadata, this.account.publicKey.toBase58());
    }

    get connection(): Connection {
        if (!this._connection) throw new Error('SolanaWalletConnect not initialised correctly');
        return this._connection;
    }

    get account(): Account {
        if (!this._account) throw new Error('SolanaWalletConnect not initialised correctly');
    ...
    public init(
        ...
        if (!this.client.controller) throw new Error('Signing client must be controller');
    ...
    async signTransaction(
        ...
        const signedTransaction = await signTransaction(this.account, transaction, this.connection);
        const response = {
            topic,
            response: {
                id: request.id,
